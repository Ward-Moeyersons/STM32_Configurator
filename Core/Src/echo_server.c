// LWIP Echo example source from:
// https://coherentmusings.wordpress.com/2012/06/11/tcp-echo-server-with-lwip/
// Slightly modified so that the function names are more consistent in the example.
// There is also a possible error on line 26+28. We will address this in class.
...
static err_t echo_accept(void *arg, struct tcp_pcb *pcb, err_t err )
{
    ...
    tcp_err( pcb, NULL );
    tcp_poll( pcb, NULL, 4 );
    tcp_write( pcb, "    Welcom to the echo server demo\r\n"
